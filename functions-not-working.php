...
/**
 * Attempt filter bar modifications
 *
...
function setup_my_field_in_bar( $filters ) {
	...
	//  Only add for list view
	if(tribe_is_view('list')){

	    $filters['tribe-bar-date-to'] = array( 
	        'name' => 'tribe-bar-date-to',
	        'caption' => 'EVENTS TO',
	        'html' => '<input type="text" name="tribe-bar-date-to" style="position: relative;" id="tribe-bar-date-to" value="" placeholder="Date"><input type="hidden" name="tribe-bar-date-to-day" id="tribe-bar-date-to-day" class="tribe-no-param" value="">'
	    );
...
function setup_my_bar_field_in_query( $query){
    ...
    if ( !empty( $_REQUEST['tribe-bar-date-to'] ) ) {
        // do stuff
        $query->query_vars['end_date'] = tribe_event_end_of_day($_REQUEST['tribe-bar-date-to']);

    }

    // TO DO : TRY TO SEARCH CUSTOM FIELDS:   Jeffrey Lee, Oceania  try single page example.



    //echo '<h1>HELLO ALEX</h1>'; 
	//var_dump($query);
	//echo '<h1>HELLO DAVID</h1>'; 
    ...
    if ($post_type == 'tribe_events') {
        ...
        $query->query_vars['meta_query'] = array(
                 array(         // restrict posts based on meta values
                    ...
                    'type'    => 'DATETIME'),
                 array(         // restrict posts based on meta values
                    'key'     => '_ecp_custom_2',  // which meta to query
                    'value'   => 'David Yoshida',  // value for comparison
                    'compare' => 'LIKE',          // method of comparison
                    'type'    => 'CHAR')                 
                 );
    }
}
...
//All the events must start after the date we filter by
function CF_search_ecp_custom_2($query) {
    $post_type = $query->query_vars['post_type']; 
    ...
    if ($post_type == 'tribe_events') {
        ...
        if ( !empty( $_REQUEST['tribe-bar-search'] ) ) {
            $keyword = $_REQUEST['tribe-bar-search'];
		...
		$array = $query->query_vars['meta_query'];

		// Define new data
		$newdata = array(         // restrict posts based on meta values
                    'key'     => '_ecp_custom_2',  // which meta to query
                    'value'   => 'David',  // value for comparison
                    'compare' => 'LIKE',          // method of comparison
                    'type'    => 'CHAR');
...
//add_filter( 'tribe_events_pre_get_posts', 'CF_search_ecp_custom_2', 100, 1 );